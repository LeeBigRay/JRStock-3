...
-(UICollectionView *)albumView
{
    if (!_albumView) {
        ...
        _albumView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, ViewSize(self.view).width, ViewSize(self.view).height-UXY_TAB_HEIGHT) collectionViewLayout:flowLayout];
...
-(LSYPickerButtomView *)pickerButtomView
{
    if (!_pickerButtomView) {
        _pickerButtomView = [[LSYPickerButtomView alloc] initWithFrame:CGRectMake(0, ViewSize(self.view).height- UXY_NAV_HEIGHT - UXY_TAB_HEIGHT, ViewSize(self.view).width, 44)];
