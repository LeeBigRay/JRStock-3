...
//        self.headerReferenceSize = CGSizeMake(kScreen_width, HeightScale(300));
    }
    return self;
}
...
- (CGSize)collectionViewContentSize {
    ...
    return CGSizeMake(self.collectionView.bounds.size.width, [_dicOfheight[maxHeightline] floatValue] + self.sectionInset.bottom + self.headerReferenceSize.height);
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //找出最短行后，计算item位置
    frame.origin = CGPointMake(self.sectionInset.left + line * (itemW + self.lineSpacing), [_dicOfheight[lineMinHeight] floatValue] +self.headerReferenceSize.height);
...
//    attr.frame = CGRectMake(x, self.headerReferenceSize.height + y, width, height );
